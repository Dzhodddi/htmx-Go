...
func (s *UsersStore) Create(ctx context.Context, tx *sql.Tx, user *User) error {
	...
	err := tx.QueryRowContext(ctx, query,
		user.Username, user.Password.hash, user.Email, r).Scan(
		&user.ID,
		&user.CreatedAt)
	if err != nil {
		switch {
...
func (s *UsersStore) Activate(ctx context.Context, token string) error {
	// find the user that this belongs to
...
func (s *UsersStore) update(ctx context.Context, tx *sql.Tx, user *User) error {
	query := `
	UPDATE users 
	SET username = $1, email = $2, is_active = $3 WHERE id = $4;`
...
func (s *UsersStore) deleteInvite(ctx context.Context, tx *sql.Tx, userID int64) error {
	query := `DELETE FROM user_invetetions WHERE user_id = $1;`
